...
class IssueTracker {
    ...
    suspend fun addComment(issueId: IssueId, comment: Comment) {
        ...
        _issueEvents.emit(AddCommentToIssueEvent(issueId, comment))
        println("Added $comment")
    }

    suspend fun addIssue(author: User, title: String): Issue {
        ...
        issues.add(newIssue)
        _issueEvents.emit(CreateIssueEvent(newIssue))
        return newIssue
    }
}
